...
// TaskEngine manages the lifecycle and periodic execution of recurring tasks.
...
func NewEngine(interval time.Duration) *TaskEngine {
	return &TaskEngine{
...
func (e *TaskEngine) Start() {
	...
	go e.processLoop()
	slog.Info("TaskEngine: Started", "interval", e.tickInterval)
}
...
func (e *TaskEngine) AddTask(t TaskFn) {
	if t == nil {
		return
	}
	select {
	case e.incoming <- t:
	default:
		slog.Warn("TaskEngine: Queue full, dropping task")
	}
}
...
func (e *TaskEngine) drainIncoming() {
	for {
		select {
		case t := <-e.incoming:
...
func (e *TaskEngine) runTaskSafely(t TaskFn, dt time.Duration) (finished bool) {
	defer func() {
		if r := recover(); r != nil {
			slog.Error("TaskEngine: TaskFn panicked", "error", r)
			finished = true
		}
	}()
	return t(dt)
